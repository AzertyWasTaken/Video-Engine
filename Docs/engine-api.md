...
> **Gotcha:** If you forget a final `_.wait()`, the last events have zero visible duration. Alternatively give the last event a `duration`.
...
_.setText(id, text, fade = 0)  // Replace text of an existing id with a crossfade
```
...
### `_.setText(id, text, fade = 0)`

Replaces the text of an existing id:
...
3. Re-advances the cursor by `fade`.
...
```js
_.setBackgroundColor(color)                 // Add a background event at the current time
...
- `clear()` sets `end` to the current time on matching events that have not ended yet; if `fadeOut` is given it overrides each event's `fadeOut`.
...
- `opts.easing` is one of `linear` (default), `quadIn`, `quadOut`, `quadInOut`, `cubicIn`, `cubicOut`, `cubicInOut` (see `Engine/easing.js`), or a custom `(t) => easedT` function.
...
- The background is the most recent background event with `start <= t` (defaults to `#000000`).
